...
double BallWalkMCMC::calculate_acceptance_rate(bool print_stats) {
...
double BallWalkMCMC::acceptance_rate() {
    return acceptance_rate_;
}
double BallWalkMCMC::acceptance_rate_deriv() {
    return acceptance_rate_deriv_;
}



void BallWalkMCMC::adjust_step_size() {
...
void BallWalkMCMC::evolve(double* point, double& loglike, double min_loglike) {
    ...
    for (curr_step_number_ = 0; curr_step_number_ < N_STEPS_PER_SAMPLE; ++curr_step_number_) {
        ...
        if (curr_step_number_ % (int)(N_STEPS_PER_SAMPLE/5) == 0) {
            adjust_step_size();
        }
        //cout << step_size() << endl;
    }
